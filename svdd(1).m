...
mu_init = -K(1,1) + SVDD.alpha'*K(:,1) +SVDD.alpha(1)*K(1,1);% SVDD.alpha'*K*SVDD.alpha;
...
for i = 1:SVDD.ntp
    SVDD.error(i) = K(i,i) - SVDD.alpha'*K(:,i) -SVDD.alpha(i)*K(i,i)  + SVDD.mu;%SVDD.alpha'*K*SVDD.alpha+SVDD.mu;
end
...
%     if(iter>10&&abs(MAX(iter)-MAX(iter-1))<0.0001)
%         break;
%     end
end;
plot(MAX);
...
mu1 = K(i1,i1) - SVDD.alpha'*K(:,i1) -SVDD.alpha(i1)*K(i1,i1);
mu2 = K(i2,i2) - SVDD.alpha'*K(:,i2) -SVDD.alpha(i2)*K(i2,i2);
if clip_alpha1 == 0
    munew = mu1;
end
if clip_alpha2 == 0
